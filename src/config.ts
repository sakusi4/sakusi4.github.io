...
/**
 * 실제로 빌드되는 로케일은 `locales` 뿐이다. 타입만 테마 원본대로 두는 이유는
 * 테마 코드 곳곳의 `locale === 'fr'` 분기가 단일 로케일 타입에서는 컴파일 에러가 나기 때문.
 */
...
export const SITE: SiteConfig = {
  ...
  description: 'Software engineer in Dubai, writing about things I build.',
  ...
  url: process.env.SITE_URL || 'https://sakusi4.github.io',
 ...
 */
/** Disqus 댓글. 예전 사이트의 스레드를 그대로 잇는다. */
export const DISQUS = {
  enabled: true,
  shortname: 'junpark',
} as const;
