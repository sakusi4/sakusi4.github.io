...
맥 주소와 포트 번호를 연결하는 방법은 여러 가지가 있겠죠. C언어에서는 크게 두 가지 겠네요. 구조체를 사용해서 묶거나 해쉬 테이블을 사용하면 됩니다. 지금은 해시테이블이 올바른 선택입니다. 두 개의 자료가 일대일 매칭이라는 점과 빠른 검색 속도가 필요하기 때문이죠. 구조체를 사용하면 시간 복잡도가 O(N)이 됩니다. 반면 해시테이블은 O(1)입니다. 
